...
class NotebookTemplate(BaseModel):
    ...
    TEMPLATE_TYPE_CHOICES = [
        ('HIGH_DIVIDEND', '高配当株ウォッチ'),
        ('GROWTH_STOCKS', '成長株リサーチ'),
        ('EARNINGS_SEASON', '決算シーズン注目'),
        ('PORTFOLIO_REVIEW', 'ポートフォリオ見直し'),
    ...
    # テンプレート設定
    default_title = models.CharField(max_length=200, verbose_name='デフォルトタイトル')
    default_strategy = models.TextField(blank=True, verbose_name='デフォルト戦略')
    default_criteria = models.JSONField(default=list, verbose_name='デフォルト選定基準')
    suggested_sub_notebooks = models.JSONField(default=list, verbose_name='推奨サブノート')
    suggested_tags = models.JSONField(default=list, verbose_name='推奨タグ')
    
    is_active = models.BooleanField(default=True, verbose_name='有効フラグ')
    
    class Meta:
        verbose_name = 'ノートブックテンプレート'
        verbose_name_plural = 'ノートブックテンプレート'
